...
export const projects: Project[] = [
  ...
  },
  {
    ...
    goal: "상용 공문서 정보추출 엔진의 품질을 층별 정량 진단으로 개선 — 병목 격리용 합성 벤치마크 구축, VLM 전환 실증, 파인튜닝 승격 게이트와 운영 추론 설정의 근거 확보",
    myRole: "품질 진단 방법론 설계 · 합성 테스트셋 구축 자동화 · E1~E6 벤치마크 설계·실행 · VLM 전환 실험 · LoRA 파인튜닝 트랙 · 운영 추론 설정(Revision) 확정",
    desc: "상용 문서 AI 엔진의 품질 개선 업무를 맡았을 때 주어진 것은 '가끔 틀린다'는 현상뿐이었습니다. 먼저 실문서 61건×5회 반복 실행 하네스를 만들어 오류를 시스템적(항상 같은 오답)·불안정·확률적으로 분류했고, 실패 케이스의 원본 이미지를 직접 확인해 '모델 행동이 아니라 모델 입력이 문제'라는 진단으로 전환했습니다. 다음 병목은 평가 그 자체였습니다. E2E 정확도 34%라는 숫자는 Detection·Recognition·필드 연결 중 어디가 문제인지 알려주지 않습니다. 법정 별지서식에 가상 값을 채우면서 그 좌표를 동시에 정답으로 저장하는 방식으로 합성 테스트셋 420장·4,732 필드를 자체 구축하고, 각 단계에 GT를 주입·제거하는 E1~E6 대체 사다리로 병목을 층별로 격리했습니다. 이 진단을 근거로 OCR 기하 규칙 파이프라인을 VLM 관계 명시형 KIE로 전환해 같은 벤치마크에서 91.7%(하이브리드 보정 시 93.3%)를 달성했고, LoRA 파인튜닝은 smoke→overfit→pilot 게이트를 통과한 것만 본 학습으로 승격하는 통제 구조로 운영했습니다. 신규 서식에서 F1이 0.339로 급락했을 때는 모델을 의심하기 전에 GT를 감사해, 오류의 72%가 표기 규칙 문제임을 밝히고 재추론 없이 재채점만으로 0.761을 회복했습니다.",
    bullets: [
      "실문서 61건×5회 반복 하네스로 오류를 시스템적·불안정·확률적으로 분류하는 진단 방법론 수립, 프롬프트 대신 ROI crop 도입으로 손글씨 필드 0/10→10/10·응답 6.3s→0.8~1.4s",
      "법정 별지서식 기반 합성 테스트셋 420장·4,732 필드 자체 구축 후 E1~E6 GT 대체 사다리 실행: GT bbox에서 Recognition 85.1% vs E2E 34.1% → Detection·필드 연결 이중 병목 격리 증명, VLM 관계 명시형 KIE 전환으로 91.7%",
      "신규 서식 F1 0.339 급락을 필드 단위 재분류로 감사 — 값 오류 151건 vs GT 표기 차이 727건(4.8배)을 밝혀 재학습 없이 재채점만으로 F1 0.761 회복",
    ],
    highlights: [
      { value: "34%→92%", label: "필드 추출 정확도" },
      { value: "4,732", label: "자체 합성 GT 필드" },
      { value: "0.34→0.76", label: "GT 감사 후 F1" },
    ],
    decisions: [
      {
        tech: "VLM 관계 명시형 KIE vs OCR + 기하 규칙 파이프라인",
        reason:
          "같은 4,732 케이스에서 PaddleOCR E2E 파이프라인은 34.1%, 위치·글자에 GT를 넣어줘도 기하 규칙의 필드 연결은 78.5%가 상한이었습니다. 반면 VLM에 '라벨-값 관계를 판단하라'는 지시만 추가한 관계 명시형 KIE는 91.7%를 기록했습니다. 반직관적인 발견은 OCR 텍스트를 VLM 프롬프트에 보조로 넣어주면 오히려 90.5%→83.0%로 떨어진다는 것이었습니다. 레이아웃 분석 모듈은 rescue 0건·regression 0건으로 완전 무효였고, 표 구조 인식만 +0.59%p로 보조 유지 가치가 있었습니다. OCR 파이프라인은 폐기하지 않고 병목 진단용 baseline으로 유지했습니다.",
      },
      {
        tech: "E1~E6 GT 대체 사다리 vs 단일 E2E 지표",
        reason:
          "E2E 34%라는 숫자만으로는 무엇을 고쳐야 하는지 알 수 없습니다. 각 단계에 정답을 주입·제거하는 실험 매트릭스를 설계했습니다: E1(전부 예측)→E2(위치만 GT)→B축(위치+글자 GT)→C축(위치 GT+OCR 글자)→E2E(전부 예측). GT bbox를 주면 Recognition이 85.1%인데 예측 박스에서는 73.1%로 떨어지는 것으로 'Detection 박스 품질이 Recognition을 끌어내린다'를, 위치·글자가 완벽해도 78.5%에 머무는 것으로 '기하 규칙 필드 연결의 구조적 상한'을 각각 독립적으로 증명했습니다. 오라클(E6)은 정의상 100%라 결과표에서 제외하고 평가기 sanity check로만 썼습니다. 부산물로 B0 실패의 51.5%가 알고리즘이 아니라 GT의 label_bbox 누락임도 드러났습니다.",
      },
      {
        tech: "VAREX 방식 합성 테스트셋 vs 사람 라벨링",
        reason:
          "사람 라벨링은 건당 5~10분, 필요 표본 150~200건이면 12~33시간이 들고 전용 검수 UI도 없었습니다. 대신 공공 포털에서 법정 별지서식을 수집해 가상 값을 프로그램으로 채우면서 채운 좌표를 동시에 GT로 저장하는 Reverse Annotation 방식을 선택했습니다 — 위치 정답이 태생적으로 정확합니다. 서식이 fillable form이 아니어서(위젯 0개) 표의 벡터 선 좌표로 격자를 복원하고 라벨 위치 기준으로 값 삽입 위치를 계산한 뒤, 실제 렌더 결과를 재측정해 GT bbox를 확보했습니다. 열화는 인쇄·노후화와 물리 촬영 2축으로 분리해 문서당 6장을 생성했고(합치면 '왜 틀렸는지'가 안 갈립니다), 순환 검증을 피하기 위해 최종 방어 수치는 사람이 확인한 실문서 홀드아웃에서만 인정한다는 원칙을 명문화했습니다.",
      },
      {
        tech: "이미지 전처리 레이어 vs 프롬프트·후처리 강화",
        reason:
          "실패 필드마다 프롬프트나 후처리 규칙을 추가하는 방식은 '이 필드에서만 참인 규칙'을 낳아 범용 엔진 원칙에 어긋났고, 공용 지시문 추가는 실측에서 효과가 0이었습니다(10/10 실패 유지, 출력 분산은 오히려 증가). 결정적 근거는 실패 케이스의 원본 이미지를 직접 확인하자 모두 '정보가 그 페이지에서 읽기 어려운 상태'였다는 것 — 모델 행동이 아니라 모델 입력의 문제였습니다. ROI crop을 도입하자 손글씨 필드가 0/10→10/10으로 뒤집혔고 응답 시간도 6.3초→0.8~1.4초로 줄었습니다. 후속 발견은 crop 폭의 최적이 필드 성격마다 다르다는 것: 손글씨는 좁은 crop, 인쇄 활자는 문맥이 포함된 넓은 crop이 이겼고, 글자 렌더링을 고정한 채 도장만 합성해 넣는 통제실험으로 '문맥 신호'가 인과적으로 작용함을 확정했습니다.",
      },
      {
        tech: "운영 추론 상한 4096 vs 3072 (max_new_tokens 스윕)",
        reason:
          "실문서의 출력은 2,000~3,000토큰으로 합성 문서의 4~6배였고, 기존 상한 1100에서는 10장 중 7~10장이 중간에 잘렸습니다. 1100→4096 스윕 결과 파인튜닝 모델은 3072에서 완전 포화(4096과 지표 동일, 지연 −22%)했지만 base 모델은 4096까지 개선이 이어졌습니다. 모델 채택이 미확정인 상태라 안전 상한 4096을 Revision 1로 승인하되 '파인튜닝 모델 확정 시 3072 하향 가능'을 조건부로 문서화했습니다. greedy 디코딩은 EOS에서 멈추므로 상한 인상은 완주 문서에 비용을 추가하지 않고, 실비용은 미완주 문서의 최악 지연뿐이라는 점도 판단 근거였습니다. 상한을 4배 올려도 반복 붕괴 3건은 하나도 해결되지 않아 '구조적 실패는 예산 문제가 아니다'를 별도 트랙으로 분리했습니다.",
      },
    ],
    experiments: [
      {
        id: "D1",
        hypothesis: "손글씨 필드 오답은 필기 자체가 애매해서 전처리로 못 고친다",
        result: "value-only crop 적용 시 10/10 정답. 원인은 손글씨가 아니라 작은 영역이 전체 페이지의 vision token 예산에 묻힌 것이었다.",
        conclusion: "필드 성격별 crop 전략 분기 도입, 응답 시간도 6.3s→0.8~1.4s 단축"
      },
      {
        id: "D2",
        hypothesis: "넓은 crop이 이기는 이유는 확대율·폰트 크기 때문이다",
        result: "글자 픽셀을 완전히 고정하고 도장만 합성해 넣자 0/10→10/10. 확대율·폰트 크기 가설은 글자 높이 실측으로 기각됐다.",
        conclusion: "문맥 신호가 인과적으로 작용함을 통제실험으로 확정, 단순 다수결 voting의 한계 확인"
      },
      {
        id: "D3",
        hypothesis: "temperature 0.0이면 출력이 결정론적이다",
        result: "540회 반복에서 동일 JSON 재현율 96.2%. 개별 호출이 아니라 세션 단위로 값이 통째로 쏠리는 현상을 확인했다.",
        conclusion: "단일 세션 반복만으로 시스템적 오류를 확정하지 않는 교차 세션 재검증 절차 도입"
      },
      {
        id: "D4",
        hypothesis: "신규 서식에서 F1 반토막(0.857→0.339)은 모델 일반화 실패다",
        result: "1,817개 필드 이벤트 재분류 결과 값 오류 151건 vs key 표기 차이 727건(4.8배). F1 0.0인 서식의 실제 값 오류는 0건이었다.",
        conclusion: "GT 표기 규칙 감사 후 재추론 없이 재채점만으로 F1 0.339→0.761 회복"
      },
      {
        id: "D5",
        hypothesis: "학습 데이터를 늘리면 문단형 필드 약점이 개선된다",
        result: "문단 정확도 0.556→0.972(+0.417). 그러나 같은 서식의 다른 필드가 붕괴하고 검증셋 F1이 허용치 초과 하락, 승격 기준 7개 중 4개 실패.",
        conclusion: "NOT_PROMOTED 판정. 원인 감사로 붕괴의 99%+가 정책 동결 이전 데이터의 구 key 표기임을 확정, 정정 후 재학습은 별도 승인 트랙으로"
      },
      {
        id: "D6",
        hypothesis: "출력 상한을 올리면 반복 붕괴도 해결된다",
        result: "상한을 4배(1100→4096) 올려도 반복 붕괴 3건 중 해결 0건.",
        conclusion: "붕괴는 예산이 아닌 구조적 실패 — 반복 감지 조기 중단을 별도 트랙으로 분리"
      },
    ],
    troubleshooting: [
      { title: "Stale Deployment — 비활성화한 코드가 계속 '동작'하는 유령 개선", body: "후처리를 추가하자 실패 필드가 10/10으로 개선됐는데, 그 코드를 일부러 비활성화해도 결과가 그대로였다. 구분 가능한 표시값을 심어 재호출하자 응답에 나타나지 않았다. uvicorn --reload가 켜져 있었지만 실제로는 파일 변경을 잡지 못하는 상태였고, 프롬프트 JSON은 서버 시작 시 1회만 캐싱되는데 reload는 .py만 감시해 JSON 변경은 원천적으로 감지되지 않았다. 재기동 후 재검증하자 '후처리 덕분'이라던 개선이 실은 프롬프트 수정만으로 달성된 것임이 드러났다. 이후 검증을 함수 단위 테스트와 서버 응답 통합 테스트로 분리하는 절차를 도입했다." },
      { title: "Session Drift — 같은 조건에서 정반대 결과", body: "한 필드가 세션1에서 10/10 실패 → 코드·프롬프트 변경 없이 세션2에서 10/10 성공으로 뒤집혔다. 원인은 API 기본 temperature 0.1 — 출력이 개별 호출이 아니라 세션 단위로 통째로 쏠릴 수 있었다. '한 세션의 반복만으로 시스템적 오류로 확정하지 않고 세션을 달리해 재확인'하는 절차를 추가하고, 30회 확정 실행을 최종 판정 근거로 삼았다. 최초의 '시스템적 실패' 판정이 방법론의 사각지대였음을 그대로 문서화했다." },
      { title: "OOM Without Traceback — 27.16GB의 3중 원인", body: "학습 스크립트가 VRAM 27.16GB(물리 24GB 초과)를 기록하고 traceback 없이 killed로 죽었다. 원인은 세 겹이었다: ① 기본 손실 경로가 전체 시퀀스 logits를 fp32로 한 번에 생성 ② sliced loss로 고친 뒤에도 로그가 27.16GB로 나온 건 max_memory_allocated가 프로세스 시작 이후 누적 최고치를 반환하는데 매 step reset을 안 했기 때문 ③ 테스트 샘플이 하필 전체 데이터 중 절대 최장 문서. sliced loss 통일 + 매 step peak reset + 샘플 선정 완화로 27.16GB→12.2GB, 이후 동일 템플릿 재사용으로 재발 0건." },
      { title: "Metric Direction Reversal — 성공률을 실패율로 인용", body: "다음 실험의 우선순위 근거로 '문단 필드 실패율 55.6%(40/72)'를 초안에 기재했는데, 집계 함수 ratio()의 반환 정의를 코드에서 확인하자 성공/전체 — 즉 40은 성공 건수, 55.6%는 성공률이었다. 잘못된 결론으로 실험을 설계하기 직전에 차단했고, 수치 인용 시 그 값을 만든 함수의 반환 정의를 코드에서 재확인하는 절차로 바꿨다. 재분석에서는 '대상 root 18개 중 13개가 열화 수준에 따라 성공·실패가 갈리는 혼재 상태'라는 더 실행 가능한 인사이트를 얻었다." },
      { title: "Stale Key Collision — 일괄 정정 스크립트의 충돌 오탐 96건", body: "'같은 문서 안에 두 필드가 같은 key를 가지면 안 된다'는 무결성 검사가 첫 실행에서 96건 충돌을 보고했다. 두 필드가 서로 맞바꾸듯 동시 정정되는 경우(환자 주소 ↔ 병원 주소), A의 새 key를 B의 정정 전(stale) key와 비교해 중간 상태를 최종 상태처럼 취급한 것이 원인이었다. 모든 필드의 최종 key를 먼저 시뮬레이션해 확정한 뒤 확정값끼리만 충돌을 검사하도록 바꿔 오탐을 96건→0건으로 제거했다." },
    ],
    insight: "정확도 단일 지표는 병목의 위치를 숨긴다 — 각 단계에 GT를 주입·제거하는 사다리가 격리 도구다. 모델을 의심하기 전에 평가 자체를 감사하라 — F1 0.339 급락의 72%는 GT 표기 규칙 문제였고 재학습 없이 재채점만으로 0.761로 회복됐다. 목표 지표가 크게 올라도(문단 정확도 0.556→0.972) 승격 게이트 7개 기준을 통과하지 못하면 배포하지 않는다 — 통제실험과 원인 감사가 '데이터를 더 넣으면 좋아진다'는 직관을 대체한다. 자동 생성 데이터로 자동 생성 모델을 평가하지 않는다 — 최종 방어 수치는 사람이 검증한 실문서 홀드아웃에서만 인정한다.",
