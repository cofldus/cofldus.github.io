...
export const projects: Project[] = [
  ...
  },
  {
    ...
    troubleshooting: [
      "BM25와 FAISS의 점수 스케일이 달라 단순 합산 시 BM25 점수가 압도적으로 높아 사실상 BM25 단독 검색과 다름없는 결과가 나왔다. 각 검색 결과의 점수를 독립적으로 Min-Max 정규화한 뒤 합산하는 방식으로 변경해 두 방식이 실질적으로 기여하는 하이브리드 구조를 확보했다.",
      "bge-reranker가 질문-문서 교차 인코딩을 40개 후보에 순차 실행하면 추론 시간이 단일 벡터 검색 대비 수배 길어져 실시간 응답 요건을 위협했다. BM25·FAISS 점수 기준 1차 필터링으로 후보를 20개로 줄인 후 reranker를 적용하고 배치 처리로 묶어 호출 오버헤드를 절감했다.",
      "5.5만 건 코퍼스 전처리 중 영문 원문에서 번역된 동일 내용이 여러 버전으로 중복 존재했다. 그대로 인덱싱하면 동일 문서가 상위를 독식해 검색 다양성이 무너진다. 청크 단위 TF-IDF 유사도 기반 중복 탐지를 추가해 인덱스 오염을 차단했다.",
      "평가셋 100문항 초안 작성 시 내과 계열 문항이 60% 이상을 차지해 외과·피부과·산부인과 영역이 과소 대표됐다. 전공자 검수 과정에서 과목별 비율을 재조정하고 난이도(기초·임상·심화)를 고르게 분배한 후 재검증했다.",
    ],
  ...
  },
  {
    ...
    troubleshooting: [
      "DART API 필드 누락과 회계 기준 차이 때문에 일부 기업은 비율 계산 자체가 실패했다. 결측값 보정과 기업별 스킵 규칙을 추가해 파이프라인 전체 중단을 막았다. API 호출 실패 시 재시도 로직(최대 3회)과 최후 수단으로 캐시된 이전 데이터를 폴백으로 활용하는 구조도 추가했다.",
      "뉴스 크롤링에서 같은 이슈가 여러 기사로 중복되는 문제가 있어, 제목·본문 유사도 기반 중복 제거를 넣지 않으면 리스크가 과장되었다.",
      "GPT-4 프롬프트는 '역할 지시(해설 전담) → XGBoost/KMeans 결과를 먼저 제시하는 컨텍스트 → 출력 포맷 → hedge 표현 규칙'의 4단 구조로 정착하기까지 수차례 A/B 비교가 필요했다. 특히 '수치를 직접 계산하지 말 것'이라는 제약과 '구체적 수치 기반 서술'이라는 요건이 충돌하는 구간이 설계에서 가장 어려운 부분이었다.",
      "XGBoost 분류 결과가 그럴듯해도 GPT가 이를 지나치게 단정적으로 표현하는 경우가 있어, '추정', '시사', '가능성' 같은 hedge 표현 규칙을 프롬프트에 추가했다.",
      "대시보드에서 숫자와 서술이 엇갈리면 신뢰도가 급격히 떨어져, 최종 표시 전 structured payload checksum 검증을 넣었다. 사용자 만족도 4.4/5.0은 경제·경영 전공자를 대상으로 시뮬레이션 리포트 3개를 직접 리뷰하게 한 후 이해도·신뢰도·사용 의향 3항목 5점 척도의 평균으로, 수치와 서술의 일치성이 신뢰도 점수에 가장 크게 기여했다.",
    ],
