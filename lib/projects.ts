...
export const projects: Project[] = [
  ...
  },
  {
    ...
    insight: "자영업자 인터뷰에서 '예측 정확도'보다 '내 조건이 바뀌면 어떻게 달라지는지'를 원한다는 말을 들었을 때, 문제 정의 자체가 달라졌다. 예측 모델은 현재 상태를 추론하지만 시뮬레이션은 가정 조건을 바꾸며 결과를 비교한다. 같은 LLM을 써도 목적에 맞는 구조가 달라야 한다는 것을 이 프로젝트에서 처음 체감했다. GPT-4.1 단일 호출로는 분석의 정확도와 전략의 창의성을 동시에 얻을 수 없었다. Temperature를 하나의 값으로 고정하면 둘 중 하나를 포기해야 하는데, 역할 자체를 분리하면 각자 최적 값에서 동작할 수 있다는 것이 2-Stage 구조의 핵심이었다. 멀티 에이전트에서 배운 것은 '에이전트 간 신뢰를 디폴트로 두면 안 된다'는 것이다. GPT-4.1이 생성한 수치를 Gemma-2가 검증 없이 받아 쓰면 환각이 파이프라인 전체로 증폭된다. 단계 경계마다 사실 검증을 끼워 넣는 구조가 멀티 에이전트 신뢰성의 최소 조건이었다.",
