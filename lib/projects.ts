...
export const projects: Project[] = [
  {
    ...
    troubleshooting: [
      { title: "RPA Selector Fragility — 페이지 구조 변경 시 전체 중단", body: "RPA 선택자가 사이트 구조 변경이나 팝업 등장에 취약했다. 강성 선택자 하나에 의존하면 페이지 레이아웃이 조금만 바뀌어도 전체가 멈췄다. CSS 선택자에 우선순위 계층을 두고, 예상 외 요소 등장 시 분기 처리하는 예외 흐름을 세분화했다." },
      { title: "Signal-to-Noise — 광고 문구와 실경험담 혼재", body: "블로그 후기에 광고성 문구와 실제 경험담이 뒤섞여 있었다. 그대로 요약하면 홍보 문구가 진로 정보처럼 제시되는 왜곡이 생겼다. 반복 패턴 감지 규칙과 키워드 필터를 추가해 신호 대 잡음비를 높였다." },
      { title: "Output Length Contract — T5 요약문 셀 초과로 리포트 파손", body: "T5 요약문이 엑셀 셀 최대 길이를 초과하면 최종 리포트가 깨졌다. 수집 단계에서는 보이지 않다가 문서 생성 직전에 터지는 문제였다. 문장 길이 상한과 강제 줄바꿈 규칙을 요약 후처리 단계에 추가했다." },
      { title: "Source Priority — 공식 포털과 후기 데이터 상충", body: "동일 학과 정보가 교육부 공식 포털과 졸업생 후기에서 상충하는 경우가 있었다. 통합 기준 없이 병합하면 신뢰도가 낮은 정보가 전면에 나올 수 있었다. 공식 데이터 > 졸업생 후기 > 보조 설명 순서의 우선순위 정책을 명문화했다." },
    ],
  ...
  },
  {
    ...
    troubleshooting: [
      { title: "Score Normalization — BM25·FAISS 점수 스케일 불일치", body: "BM25와 FAISS의 점수 스케일이 달라 단순 합산 시 BM25 점수가 압도적으로 높아 사실상 BM25 단독 검색과 다름없는 결과가 나왔다. 각 검색 결과의 점수를 독립적으로 Min-Max 정규화한 뒤 합산하는 방식으로 변경해 두 방식이 실질적으로 기여하는 하이브리드 구조를 확보했다." },
      { title: "Reranker Batching — 교차 인코딩 순차 처리 병목", body: "bge-reranker가 질문-문서 교차 인코딩을 40개 후보에 순차 실행하면 추론 시간이 단일 벡터 검색 대비 수배 길어져 실시간 응답 요건을 위협했다. BM25·FAISS 점수 기준 1차 필터링으로 후보를 20개로 줄인 후 reranker를 적용하고 배치 처리로 묶어 호출 오버헤드를 절감했다." },
      { title: "Index Contamination — 중복 문서 검색 다양성 붕괴", body: "5.5만 건 코퍼스 전처리 중 영문 원문에서 번역된 동일 내용이 여러 버전으로 중복 존재했다. 그대로 인덱싱하면 동일 문서가 상위를 독식해 검색 다양성이 무너진다. 청크 단위 TF-IDF 유사도 기반 중복 탐지를 추가해 인덱스 오염을 차단했다." },
      { title: "Evaluation Bias — 평가셋 과목 분포 편향", body: "평가셋 100문항 초안 작성 시 내과 계열 문항이 60% 이상을 차지해 외과·피부과·산부인과 영역이 과소 대표됐다. 전공자 검수 과정에서 과목별 비율을 재조정하고 난이도(기초·임상·심화)를 고르게 분배한 후 재검증했다." },
      { title: "Offline-Online Gap — RAGAS Faithfulness 도입", body: "오프라인 평가셋 BERTScore가 양호해도 실제 질문 분포가 달라지는 온라인 환경에서 체감 품질 괴리가 생겼다. Offline-Online Gap 문제를 인식하고 RAGAS Faithfulness 메트릭을 도입해 생성 답변이 검색 컨텍스트에 얼마나 충실한지를 별도 측정했다. 전문의 검수를 거친 Factual Consistency 골드셋 100문항으로 오프라인 평가 기준 자체를 실제 사용 분포에 맞게 재조정했다." },
      { title: "Pipeline Latency Budget — 3단 레이턴시 예산 배분", body: "Candidate Generation(BM25+FAISS) → Reranking → LLM 생성의 3단 파이프라인에서 단계별 레이턴시 예산을 명시하지 않으면 최적화가 서로를 잠식했다. 각 단계에 ≤30ms / ≤50ms / ≤120ms 예산을 고정하고 역방향으로 병목을 추적했다. Reranker가 후보 40개 기준 82ms로 예산을 초과해, 1차 필터링으로 후보를 20개로 줄이고 배치 처리로 전환해 48ms로 단축했다." },
    ],
  ...
  },
  {
    ...
    troubleshooting: [
      { title: "Multi-Type Interference — 혼합 난독화 단일 모델의 전략 충돌", body: "기본 난독화와 야민·그래픽형이 한 문장에 동시에 섞이면, 단일 모델은 어느 복원 전략을 먼저 적용할지 일관되게 결정하지 못했다." },
      { title: "Sparse Semantic Space — 자모 분리 입력의 의미 공간 희박화", body: "자모 분리 입력은 형태 복원에 유리했지만, 의미 공간이 희박해져 긴 입력에서 새로운 문장을 만들어내는 부작용이 있었다." },
      { title: "Prompt vs. Internalization — 프롬프트 제약의 토큰 비용 딜레마", body: "프롬프트는 즉각적인 제약 효과가 있었으나 토큰을 소모하고 긴 리뷰에서는 오히려 집중력 저하처럼 작동했다. 최종 통합 구조는 프롬프트를 제거하고 학습 내재화를 택했다." },
      { title: "Over-Generation — EOS 이후 과생성 후처리 통제", body: "EOS 이후 과생성이 모델 학습만으로 해결되지 않아, 입력 문장 수 기준 출력 제한 Lock과 1.3배 초과 시 강제 컷 규칙을 후처리로 추가했다." },
    ],
  ...
  },
  {
    ...
    troubleshooting: [
      { title: "Quantization Quality — 양자화 강도와 생성 품질 트레이드오프", body: "양자화 강도를 높이면 교정문은 유지되어도 설명 문장의 문법 메타언어가 흔들렸다. 학습자 서비스에서 이 부분이 체감 품질을 크게 해쳤다. lm head와 일부 attention 경로를 보수적으로 유지했다." },
      { title: "RAG Over-Correction — 예문 다양성 부족으로 인한 과수정", body: "RAG에서 너무 비슷한 예문만 주면 모델이 패턴을 복사해 사용자 문장을 과수정하는 현상이 있었다. 검색 다양성 제약을 추가했다." },
      { title: "Memory Contamination — 세션 간 오류 패턴 오염", body: "개인화 메모리가 강하게 개입하면 이전 오류 유형이 현재 문장에도 과도하게 적용되는 메모리 오염 문제가 있었다. 세션 감쇠 규칙을 추가해 완화했다." },
      { title: "FAISS HNSW → IVF — 검색 병목 구조 재설계", body: "UiPath에서 Python을 직접 호출하는 방식은 라이브러리 경로·버전 차이로 잦은 실패를 일으켰다. FAISS HNSW → IVF 구조로 변경 후 유사 예문 검색 0.21s → 0.07s로 단축됐다." },
      { title: "Catastrophic Forgetting — LoRA 파라미터 격리와 리플레이", body: "LoRA 파인튜닝 도중 기존 일반 한국어 생성 능력이 열화되는 Catastrophic Forgetting이 관찰됐다. EWC(Elastic Weight Consolidation)를 검토했으나 중요도 행렬 계산 비용이 온디바이스 제약과 맞지 않아 채택하지 않았다. LoRA 특성상 원본 가중치를 동결하고 delta 행렬만 학습하는 파라미터 격리 구조가 자연스러운 망각 방지 역할을 했으나, 학습 데이터에 일반 한국어 예문을 혼합하는 리플레이 방식으로 추가 보완했다." },
      { title: "Tiered Caching — T1·T2·T3 계층 캐시로 P99 레이턴시 달성", body: "P99 응답 지연 목표(200ms)를 맞추기 위해 요청 흐름을 3단 캐시 계층으로 설계했다. T1 인메모리 LRU(최근 200 쿼리) → T2 SQLite 디스크 캐시(30일 보존) → T3 FAISS IVF 의미 검색으로 이어지는 Tiered Caching 구조다. 동일 쿼리의 T1 히트율 37%가 전체 평균 응답 시간을 40% 단축했고, 단일 FAISS 검색만 사용했을 때 대비 P99 레이턴시가 310ms → 180ms로 개선됐다." },
    ],
  ...
  },
  {
    ...
    troubleshooting: [
      { title: "DART API Retry — 결측·회계 기준 차이로 인한 파이프라인 중단", body: "DART API 필드 누락과 회계 기준 차이 때문에 일부 기업은 비율 계산 자체가 실패했다. 결측값 보정과 기업별 스킵 규칙을 추가해 파이프라인 전체 중단을 막았다. API 호출 실패 시 재시도 로직(최대 3회)과 최후 수단으로 캐시된 이전 데이터를 폴백으로 활용하는 구조도 추가했다." },
      { title: "News Dedup — 중복 기사로 인한 리스크 과장", body: "뉴스 크롤링에서 같은 이슈가 여러 기사로 중복되는 문제가 있어, 제목·본문 유사도 기반 중복 제거를 넣지 않으면 리스크가 과장되었다." },
      { title: "Prompt A/B — 역할·제약 충돌 구간 4단 구조 정착", body: "GPT-4 프롬프트는 '역할 지시(해설 전담) → XGBoost/KMeans 결과를 먼저 제시하는 컨텍스트 → 출력 포맷 → hedge 표현 규칙'의 4단 구조로 정착하기까지 수차례 A/B 비교가 필요했다. 특히 '수치를 직접 계산하지 말 것'이라는 제약과 '구체적 수치 기반 서술'이라는 요건이 충돌하는 구간이 설계에서 가장 어려운 부분이었다." },
      { title: "Hallucination Hedge — LLM 단정 표현 통제", body: "XGBoost 분류 결과가 그럴듯해도 GPT가 이를 지나치게 단정적으로 표현하는 경우가 있어, '추정', '시사', '가능성' 같은 hedge 표현 규칙을 프롬프트에 추가했다." },
      { title: "Payload Checksum — 수치·서술 불일치 신뢰도 붕괴", body: "대시보드에서 숫자와 서술이 엇갈리면 신뢰도가 급격히 떨어져, 최종 표시 전 structured payload checksum 검증을 넣었다." },
      { title: "Concept Drift · PSI — 피처 분포 이동과 자동 재학습 트리거", body: "시장 사이클·업종 재편에 따라 피처 분포가 서서히 이동하는 Concept Drift가 발생했다. 6개월 후 재운영 시 XGBoost 등급 분포가 학습 시점과 눈에 띄게 달라졌다. 주요 피처별 PSI(Population Stability Index)를 산출해 PSI > 0.25를 초과하는 피처가 2개 이상이면 자동으로 재학습을 트리거하는 모니터링 파이프라인을 추가했다." },
  ...
  },
  {
    ...
    troubleshooting: [
      { title: "PDF Layout Parsing — 표·수식 파손으로 청크 오염", body: "강의 PDF 파싱 시 표와 수식이 깨져 의미 없는 기호 열이 됐다. PyMuPDF 대신 레이아웃 분석을 지원하는 pdfplumber로 교체하고, 수식 포함 셀은 플래그를 달아 퀴즈 생성 대상에서 제외했다." },
      { title: "Reproducibility — 동일 쿼리에 매번 다른 퀴즈 생성", body: "동일한 질문에 매번 다른 퀴즈가 생성돼 강사 검토·재활용이 불가능했다. temperature를 0.3으로 낮추고 동일 청크 결과를 캐싱하는 레이어를 추가해 재현성을 확보했다." },
      { title: "Summarization Depth — 가이드가 원문 단순 반복에 머무름", body: "학습 가이드 요약이 강의 자료를 단순 반복하는 수준에 머물렀다. '핵심 개념 → 이유·원리 → 예시' 순서로 재구성하도록 프롬프트 구조를 명세화하고 출력 형식을 JSON으로 강제했다." },
    ],
