...
export const projects: Project[] = [
  {
    ...
    desc: "회사 제품인 문서 인식 엔진은 증명서와 진단서 사진에서 이름, 날짜, 금액 같은 항목을 추출해 시스템에 전달합니다. 처음 확인된 것은 일부 항목에서 오류가 발생한다는 현상뿐이었습니다.\n\n실제 문서 61건을 5회씩 반복 실행하는 채점 도구를 만들어, 항상 같은 오답이 나오는 항목과 실행마다 결과가 달라지는 항목을 구분했습니다. 오답이 나온 항목의 원본 이미지를 확인한 결과 대부분 해당 정보가 페이지에서 작게 표시되어 있었습니다. 모델의 판단 이전에 입력 이미지 쪽에 원인이 있었습니다.\n\n다음 문제는 측정 방법이었습니다. 이 엔진은 값의 위치를 찾고, 글자를 읽고, 읽은 값을 항목에 연결하는 세 단계로 동작합니다. 최종 정확도 34%라는 값만으로는 세 단계 중 어디에서 손실이 발생하는지 알 수 없습니다.\n\n실제 문서에는 정답 좌표가 없어 단계별 측정이 불가능했습니다. 정부가 공개한 법정 서식에 가짜 값을 채우면서 그 좌표를 함께 저장하는 방식으로 정답지가 포함된 합성 문서 420장, 4,732문항을 만들었습니다.\n\n단계마다 정답을 주입해 측정한 결과는 다음과 같았습니다. 위치를 정답으로 주면 글자 인식이 85.1%, 위치와 글자를 모두 정답으로 줘도 항목 연결이 78.5%, 전부 예측하면 34.1%였습니다. 글자 인식보다 위치 탐지와 항목 연결에서 손실이 컸습니다.\n\n이 결과를 근거로 인식 방식을 문서 전체를 입력받는 VLM 기반으로 변경해 같은 문서에서 91.7%가 됐습니다. 이후 모델 파인튜닝은 소규모 검증을 통과한 경우만 본 학습으로 진행하고, 운영 설정 변경은 스윕 실험 결과를 근거로 확정했습니다.",
    bullets: [
  ...
  },
  {
    ...
    desc: "FAISS 벡터 검색만 쓰는 기본 구성으로 프로토타입을 만들고 국가고시 기출 문제로 평가했습니다. 답변은 나왔지만 근거 문서가 맞지 않는 경우가 있었습니다.\n\n'폐색전증 항응고 치료 시작 기준'을 질의하면 '항응고제 일반 개요'나 '혈전예방 가이드라인 서문'처럼 주제는 같지만 임상 수치가 없는 문서가 상위에 올랐습니다. D-dimer와 INR 기준이 기재된 문서는 순위 밖이었습니다. 벡터 검색은 두 문서가 '혈전 관련 치료'라는 주제에서 가깝다고 판단하고, 구체적인 기준치가 있는지는 반영하지 못합니다.\n\n정확한 단어 일치를 보완하려고 BM25를 단독으로도 시험했습니다. 이번에는 영문 원문을 한국어로 풀어 쓴 문서가 누락됐습니다. 같은 개념도 표기가 다양해 단어가 일치하지 않았기 때문입니다.\n\n두 방식이 서로 다른 유형의 문서를 놓친다는 것을 확인하고 병렬로 실행해 후보 40건을 모았습니다. 후보를 모두 LLM에 넣었을 때는 더 적게 넣었을 때보다 BERTScore가 낮았습니다. 관련 없는 문서가 답변에 인용됐기 때문입니다. 질문과 문서를 함께 인코딩해 관련도를 다시 계산하는 Cross-Encoder(bge-reranker)로 상위 5건만 전달하도록 바꿨습니다.",
    bullets: [
  ...
  },
  {
    ...
    desc: "필터를 우회하려고 변형한 리뷰 텍스트를 원래 표기로 되돌리는 프로젝트입니다. 변형 유형은 두 가지입니다. 야민정음은 '머'를 '뫄'처럼 모양이 비슷한 글자로 바꾸고, 음운 오류는 '됩니다'를 '됩니당'처럼 발음이 비슷한 표기로 바꿉니다.\n\n두 유형은 복원할 때 참고하는 정보가 다릅니다. 야민정음은 글자 모양의 대응 관계를, 음운 오류는 발음과 표기의 대응 관계를 사용합니다.\n\n하나의 KoBART 모델로 둘을 함께 학습시키자 두 유형이 섞인 문장에서 출력이 일정하지 않았습니다. 학습 데이터를 3배로 늘려도 오류율이 줄지 않아, 데이터 양이 아니라 구조를 바꾸기로 했습니다.\n\nKoELECTRA 분류기가 변형 유형을 먼저 판별하고 해당 유형 전용 모델로 전달하는 구조로 변경했습니다. 각 모델이 한 가지 유형만 학습하면서 출력이 안정됐습니다.",
    bullets: [
  ...
  },
  {
    ...
    card: {
      ...
      oneliner: "LoRA·4bit 양자화와 FAISS 개인화 메모리로 end-to-end 0.47초 응답을 구현한 콩글리시 교정 에이전트",
    },
    num: "04",
    title: "온디바이스 콩글리쉬 교정 에이전트",
    subtitle: "14GB 모델을 3.5GB로 경량화하고 ARM CPU end-to-end 응답 시간을 0.47초로 단축",
    period: "2025.07–08 · 포스코 AI·BigData 아카데미 30기",
    goal: "14GB 다국어 LLM을 3.5GB로 압축해 CPU에서 end-to-end 0.47초 이내 응답, 모바일 환경에서도 콩글리쉬 실시간 교정 가능한 Agent 구현",
    myRole: "LoRA + 4-bit 양자화 + 토큰 Pruning 조합 설계 · FAISS/SQLite 하이브리드 메모리 구현 · 응답 지연 최적화 전 과정",
    desc: "'블랙 컨슈머', 'after 신청'처럼 한국인 특화 콩글리쉬는 LLM 학습 데이터에 거의 등장하지 않아 교정 결과가 일정하지 않았습니다.\n\n서비스를 Android 단말에서 동작시키기로 하면서 제약이 하나 더 생겼습니다. 14GB 모델을 ARM CPU에서 실행해야 했습니다.\n\n모델 압축과 지식 분리를 함께 설계했습니다. LLM은 문법과 문맥 판단만 수행하고, 콩글리쉬 용례는 별도 벡터 DB에서 검색해 전달합니다.",
    bullets: [
      "모델 압축: LoRA + 4-bit NF4 양자화 + 멀티링구얼 토큰 Pruning(~10%) 조합으로 14GB→3.5GB(75%), 모델 단독 추론 0.3s(Snapdragon 865 ARM CPU)",
      "지식 분리: FAISS(IVF) + SQLite LRU 캐시 하이브리드로 630개 콩글리쉬 패턴 DB를 0.02초 내 검색; 콩글리쉬 패턴 DB는 SNS·커뮤니티 크롤링 + 팀 수기 검수로 구축",
      "end-to-end 응답 지연 2.3s→0.47s(평균, 5배 단축, Snapdragon 865 ARM CPU 측정), 모바일 일관성 92%+ 확보; cProfile로 병목이 LLM이 아닌 FAISS 검색(0.21s)·SQLite I/O(0.12s)임을 실측 후 최적화",
    ],
    highlights: [
      { value: "75%", label: "모델 경량화 14GB→3.5GB" },
      { value: "0.47s", label: "end-to-end 응답 평균" },
    ...
    troubleshooting: [
      ...
      { title: "Catastrophic Forgetting — LoRA 파라미터 격리와 리플레이", body: "LoRA rank=16으로 콩글리쉬 특화 파인튜닝 진행 중 학습 2 epoch 이후부터 일반 한국어 문장 생성 품질이 저하됐다. perplexity 측정 결과 일반 한국어 코퍼스 기준 사전 학습 모델 대비 18% 상승 — Catastrophic Forgetting의 전형적 징후였다. EWC(Elastic Weight Consolidation)를 검토했으나 Fisher Information Matrix 계산이 온디바이스 메모리 제약과 맞지 않아 채택하지 않았다. LoRA의 원본 가중치 동결 + delta 행렬만 학습하는 파라미터 격리 구조가 기본적인 망각 방지 역할을 했지만, 콩글리쉬 데이터 비율이 90% 이상일 때는 부족했다. 학습 배치에 일반 한국어 예문을 20% 혼합하는 리플레이 방식을 추가해 일반 문장 품질을 사전 학습 기준 97% 수준으로 회복했다." },
      { title: "Tiered Caching — T1·T2·T3 계층 캐시로 지식 검색 경로 P99 단축", body: "지식 검색 경로의 P99 목표를 200ms로 잡으면 LLM 추론 시간을 빼고 남는 예산이 50ms에 불과했다. 매 요청마다 FAISS를 호출하면 검색 단계에서만 70ms가 소요돼 목표 달성이 불가능했다. 요청 유형 분석 결과 동일 단어·표현에 대한 반복 질의가 전체의 약 55%를 차지했다. T1 인메모리 LRU(최근 200 쿼리, <1ms) → T2 SQLite 디스크 캐시(30일 보존, ~5ms) → T3 FAISS IVF(~70ms)의 3단 캐시 계층을 설계했다. T1 히트율 37%, T2 히트율 18%로 전체 요청의 55%가 FAISS를 거치지 않았고, 지식 검색 경로 P99 레이턴시가 310ms → 180ms로 개선됐다. (end-to-end 응답 평균 0.47s와는 측정 구간이 다르다)" },
    ],
  ...
  },
  {
    ...
    insight: "LLM에게 수치 계산을 위임하면 설득력 있는 오답이 나온다 — 환각 80% 감소의 핵심은 XGBoost 등급·KMeans 군집 평균·리스크 태그를 비LLM 단계에서 먼저 계산해 구조화된 컨텍스트로 주입하고 LLM은 서사적 해설만 담당하게 하는 역할 분리였다. 컨텍스트 규모와 응답 품질은 단조 증가하지 않는다 — 뉴스 원문 전체보다 위험 키워드·긍정 이벤트·불확실 이슈로 사전 분류된 구조적 요약이 GPT 응답 정합성을 높였다. 비동기 파이프라인에서 수치 불일치 신뢰도 붕괴는 Checksum 검증(XGBoost 산출 수치 ↔ GPT 서술 추출 수치 1:1 매칭)으로만 차단 가능하다. Training-Serving Skew의 근원은 Point-in-Time 미적용이다 — 피처 계산 기준 날짜를 Feature Store에 명시 기록해야 학습·서빙 피처 정의가 일치한다.",
  ...
  },
  {
    ...
    desc: "CT와 MRI 사이를 변환하는 모델을 학습시키는 연구입니다.\n\n같은 환자의 CT와 MRI를 같은 시점에 촬영한 Paired 데이터는 임상에서 확보하기 어렵습니다. 비용과 촬영 절차상의 제약 때문입니다. 그래서 짝이 맞지 않는 데이터로 학습하는 Unpaired Image-to-Image Translation으로 접근했습니다.\n\nCycleGAN은 변환한 이미지를 원래 도메인으로 되돌렸을 때 원본과 일치하도록 학습시켜, 짝 정보 없이 두 도메인의 대응 관계를 학습합니다. 학습 과정에서는 Generator와 Discriminator의 수렴 속도 차이로 출력이 무너지는 현상이 반복돼, 학습률과 업데이트 비율을 조정하며 안정화했습니다.",
    bullets: [
    ...
    insightLabel: "결과 해석",
    insight: "학습 초기에 Discriminator가 Generator보다 먼저 수렴하면서 Generator가 gradient를 받지 못했고, 출력이 몇 가지 형태로만 수렴하는 모드 콜랩스가 반복됐다. Generator 학습률을 2배로 두고 업데이트 비율을 G:D=2:1로 조정한 뒤 수렴했다. Spectral Normalization은 Discriminator 가중치의 변화 폭을 제한해 gradient가 과도하게 커지는 것을 억제했다.\n\nLoss 구성에서는 픽셀 단위 L1 Loss만 사용했을 때 전체 윤곽은 재현됐으나 뼈 경계와 장기 윤곽이 흐리게 생성됐다. VGG 중간 레이어의 feature map을 비교하는 Perceptual Loss를 추가하자 해부학적 경계 유실이 30% 감소했다. L1은 픽셀값 차이만 보기 때문에 평균적으로 안전한 흐린 출력을 만드는 반면, feature map 비교는 구조가 유지됐는지를 평가한다.\n\nPaired 데이터 없이 변환이 가능한 근거는 Cycle-Consistency Loss다. 변환한 이미지를 원래 도메인으로 되돌렸을 때 원본과 일치하도록 학습시켜, 두 도메인의 대응 관계를 페어 없이 학습한다.",
  ...
  },
  {
    ...
    insightLabel: "설계 판단",
    insight: "추천 점수를 단일 지표로 계산했을 때 결과가 한쪽으로 치우쳤다. 혼잡도만 쓰면 사람이 없는 장소를, 거리만 쓰면 방문 목적과 맞지 않는 장소를 추천했다. 혼잡도 40%, 거리 30%, 장소 유사도 20%, 시간대 10%로 정규화한 값을 가중 합산했고, 혼잡도 비중을 가장 높게 둔 것은 밀집 분산이 서비스 목적이기 때문이다.\n\nFP-Growth로 이동 로그의 연관 규칙을 추출하면 거리만으로는 알 수 없는 실제 이동 경로 패턴이 나온다. 이 패턴을 추천 근거로 함께 제시했다.\n\n오프라인 SQLite 캐시는 기능이 아니라 전제 조건으로 넣었다. 인파가 몰리는 시점에는 네트워크 요청이 증가하거나 끊길 수 있어, 서버 응답이 없어도 마지막으로 받은 히트맵과 추천으로 동작해야 했다.",
  ...
  },
  {
    ...
    desc: "자영업자 15명을 인터뷰한 결과, 매출 예측값보다 매장 조건을 바꿨을 때의 변화를 확인하고 싶다는 요구가 반복해서 나왔습니다. 그래서 매출을 예측하는 대신 조건을 바꿔 실행해 보는 시뮬레이션으로 설계했습니다.\n\n조건별 고객 반응은 160개의 AI 페르소나가 각각 판단합니다. 각 페르소나는 인지, 관심, 방문 검토, 결정, 재방문의 5단계를 거치며, 단계를 나눈 덕분에 최종 결과뿐 아니라 어느 단계에서 이탈했는지도 확인할 수 있습니다.\n\nLLM 호출은 두 단계로 나눴습니다. Temperature를 하나로 고정하면 수치 정리와 전략 제안 중 한쪽이 나빠져서, 앞 단계는 낮은 값으로 상권 수치를 정리하고 뒷 단계는 높은 값으로 전략을 제안하도록 했습니다.",
    bullets: [
    ...
    troubleshooting: [
      { title: "2-Stage Output Coupling — 분석가 포맷이 전략가 다양성을 억제", body: "T=0.1 분석가 모듈의 출력을 그대로 T=0.7 전략가 모듈에 전달하면, 전략가가 분석가의 구조화된 포맷과 표현에 구속되어 전략 제안 다양성이 기대치 이하로 수렴하는 현상이 나타났다. '영업시간 연장'이라는 전제 조건에 대해 전략 옵션이 항상 3개 이하로 나오고 내용도 유사했다. 분석가 출력을 키-값 JSON 중간 표현으로 정규화한 뒤 전략가에게는 포맷 없이 사실 데이터만 전달하는 인터페이스 레이어를 삽입했다. 전략가가 형식 제약 없이 수치만 참조하게 되자 제안 옵션 평균 수가 2.4개 → 5.1개로 늘었고 내용 다양성도 함께 개선됐다." },
      { title: "Persona Diversity Collapse — 160 페르소나의 방문 결정 수렴", body: "160개 페르소나가 독립 시나리오를 실행할 때 방문 결정 단계에서 다수가 동일한 '방문함' 결정으로 수렴하는 현상이 발생했다. 방문 목적 4유형 × 세대 5종으로 조합을 나눴음에도 실제 결정 분포가 80% 이상 '방문함'으로 편중됐다. 원인은 페르소나 정의에 가격 민감도, 이동 거리 임계값, 피로도 같은 행동 억제 변수가 없었기 때문이었다. 각 페르소나에 가격 저항도·거리 허용치·사회적 영향 수용도 3개 행동 다양성 파라미터를 추가했다. 보정 후 방문 결정 분포가 방문 47% / 보류 31% / 이탈 22%로 실제 상권 방문 패턴에 근접했다." },
      { title: "Hallucination Propagation — 에이전트 간 수치 환각 전파", body: "GPT-4.1이 생성한 '예상 매출 증가율 18%' 같은 수치를 Gemma-2가 검증 없이 다음 단계 컨텍스트로 채택하는 오류 전파가 발생했다. 멀티 에이전트 파이프라인에서 앞 단계 출력은 암묵적으로 신뢰되기 때문에 환각이 파이프라인 전체로 증폭됐다. 최종 리포트에 실제 상권 데이터 기반 수치와 다른 값이 등장해도 어느 에이전트가 생성했는지 추적이 불가능했다. 각 에이전트 출력의 수치를 실제 상권 입력 데이터와 대조하는 fact-check 레이어를 에이전트 간 전달 경계마다 삽입했다. ±20% 범위를 벗어나는 수치는 '추정값'으로 명시 태깅하고 최종 리포트에 신뢰 구간을 함께 표시하도록 변경했다." },
      { title: "Simulation Throughput — 160 페르소나 순차 처리 병목", body: "160개 페르소나를 순차적으로 LLM 호출하면 시뮬레이션 1회 실행에 평균 4분 22초가 소요됐다. 자영업자가 조건을 바꿔가며 반복 시나리오를 돌리는 실제 사용 패턴에서 이 지연은 서비스 사용을 포기하게 만드는 수준이었다. 페르소나 간 의존성이 없는 독립 의사결정 구조임을 확인하고 asyncio 기반 배치 병렬 처리(20개씩 동시 실행)로 전환했다. 동시 호출 수는 API 레이트 리밋(TPM) 상한을 역산해 배치 크기를 결정했다. 병렬 처리 전환 후 전체 시뮬레이션 시간이 4분 22초 → 48초로 단축됐다." },
    ],
  ...
  },
  {
    ...
    insightLabel: "리서치 노트",
    insight: "파서 선택은 수식 처리에서 갈린다. 일반 PDF 파서는 첨자와 분수를 깨진 문자로 변환하지만, Nougat과 Marker 같은 VLM 기반 파서는 렌더링된 수식을 LaTeX로 복원한다. 이 차이가 학습 데이터 품질의 상한을 결정한다.\n\n과학 데이터만으로 CPT(계속 사전학습)를 수행하면 기존에 학습한 일반 언어 능력이 저하되는 Catastrophic Forgetting이 발생한다. 일반 도메인 데이터를 15~20% 함께 사용하는 비율이 저하 억제와 도메인 성능 사이에서 확인한 균형점이었다.\n\n사실 정확도, 형식 준수, 응답 간결성을 하나의 보상 값으로 합산하면 한 항목이 올라갈 때 다른 항목이 내려간다. GDPO는 각 보상 신호를 그룹 안에서 개별 정규화해 이 상충을 완화하는 방식이다.",
  ...
  },
  {
    ...
    desc: "강의 영상과 자료를 업로드하면 개념 퀴즈와 학습 가이드를 생성하는 시스템입니다. 강사가 수업 자료를 정리하고 출제하는 데 쓰던 시간을 줄이는 것이 목표였습니다.\n\n초반에는 강의 자료로 모델을 파인튜닝하는 방향도 검토했습니다. 강의는 매 학기 새로 올라오고 내용도 달라져 매번 재학습이 필요했고, 이 비용이 운영에 맞지 않았습니다.\n\n강의 문서를 청크로 나눠 FAISS에 인덱싱하고 LangChain RetrievalQA로 문항을 생성하는 구성으로 바꿨습니다. 새 강의는 인덱스만 교체하면 되고 재학습이 필요 없습니다.",
    bullets: [
  ...
  },
  {
    ...
    desc: "AI 기술을 사용하지 않는 풀스택 프로젝트입니다. Next.js, TypeScript, WebSocket 개발 경험을 보여주기 위해 포함했습니다.\n\n약속을 정할 때 지도 링크를 공유하고, 투표 링크를 따로 보내고, 다시 채팅으로 돌아오는 과정을 반복하게 됩니다. 이 과정에서 대화 흐름이 계속 끊깁니다.\n\nMOIM은 장소 검색부터 후보 핀 등록, 실시간 투표, 모임 확정까지를 채팅 안에서 처리합니다. Next.js 15 App Router와 STOMP WebSocket으로 실시간 동기화를 구현하고, Kakao Maps API로 장소 핀을 채팅 메시지와 같은 인터페이스에 통합했습니다.",
    bullets: [
    ...
    insightLabel: "구현 노트",
    insight: "Zustand 스토어를 컴포넌트 상단에서 초기화했을 때 Hydration Mismatch가 발생했다. 서버 렌더링 시점에는 window 객체가 없어 URL 파라미터를 읽지 못하고, 클라이언트에서 마운트된 뒤에야 값이 설정되기 때문이다. 초기화 코드를 useEffect 안으로 옮겨 클라이언트에서만 실행하도록 했다.\n\nKakao Maps SDK는 함수 호출로 마커를 생성하고 제거하는 방식이라 React의 렌더링 방식과 맞지 않았다. 마커 인스턴스를 useRef에 보관하고 생성·제거를 useEffect 안에서 처리해, 지도 인스턴스를 다시 만들지 않고 마커만 갱신하도록 했다.\n\n모바일에서는 지도 핀 선택과 채팅 스크롤이 같은 영역에서 발생해 스크롤 중 핀이 선택되는 경우가 있었다. 터치 시작과 종료 좌표의 이동 거리가 10px 이하일 때만 탭으로 처리하는 조건을 추가했다.",
