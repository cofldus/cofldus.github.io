...
export const projects: Project[] = [
  {
    ...
    insight: "RPA 안정성의 핵심은 선택자 단일화가 아니라 계층형 폴백 설계다. CSS 경로 고정 → 대체 선택자 → 텍스트 매칭의 3단 우선순위 계층은 개별 학과 실패를 격리해 나머지 수집이 계속되도록 한다. T5 요약 품질 개선보다 이 격리 구조가 실제 안정성에 더 크게 기여했다. 파이프라인 단계 간 '중간 표현 계약'을 명시하지 않으면 각 단계는 개별적으로 정상이어도 통합 시 깨진다. T5 토큰 상한(512), 엑셀 셀 용량(32,767자), 선택자 수명은 각 단계 경계를 명시적으로 정의해야 제거되는 계약 위반이었다. 소스 이질성은 추상화가 아니라 도구 선택으로 흡수한다 — 브라우저를 직접 구동하는 RPA는 소스별 별도 파서 구현 없이 로그인·동적 로딩을 일괄 처리했다.",
  ...
  },
  {
    ...
    insight: "BM25와 FAISS의 점수 스케일 불일치(TF-IDF 절대값 vs 코사인 유사도 0~1)를 해소하지 않으면 하이브리드가 단독 검색보다 나쁠 수 있다 — 각 시스템 독립 Min-Max 정규화 후 가중 합산(α=0.4/β=0.6)이 선행 조건이었다. 교차 인코딩 리랭커(bge-reranker-v2-m3)의 병목은 후보 수에 선형 비례한다 — 40건 순차 처리 820ms는 20건 1차 컷오프 + 배치 단일 호출로 210ms로 줄었다. 컨텍스트 크기와 LLM 응답 품질은 단조 증가하지 않는다 — Top-40보다 Top-5가 BERTScore가 높았다. 단계별 레이턴시 예산(≤30ms/≤50ms/≤120ms)을 선설정하고 역방향 프로파일링해야 전체 목표(200ms)를 달성할 수 있다 — 전체 목표만 설정하면 한 단계의 최적화 여지가 다른 단계에 흡수된다.",
  ...
  },
  {
    ...
    insight: "단일 seq2seq 모델이 야민정음·음운 오류를 동시에 처리하지 못하는 이유는 데이터 부족이 아니라 복원 기제의 근본 차이다 — 그래픽형은 자모 형태 매핑, 음운형은 음성 역변환이다. 데이터 3배 증량으로 혼합형 오류율이 줄지 않는 것을 확인하고 구조 설계 문제로 재진단했다. JAMO 전처리는 그래픽 노이즈 형태 복원에는 유효하지만 단독 적용 시 어절 경계 소실로 임베딩 공간이 희박해져 긴 입력에서 환각을 유발한다 — 분기형 구조의 그래픽 경로에만 선택적으로 적용되는 조건부 전처리다. KoELECTRA 분류기의 실질적 역할은 판단 책임을 분산해 각 복원 모델이 단일 방향 손실 함수에만 집중하도록 하는 것이다 — 이 구조가 베이스라인 대비 BERTScore +5.6%p 개선의 근거다.",
  ...
  },
  {
    ...
    insight: "온디바이스 추론에서 병목은 직관과 반대로 LLM 추론이 아니라 FAISS 검색(0.21s)과 SQLite 조회(0.12s)였다 — cProfile 실측 없이는 잘못된 방향으로 최적화한다. FAISS HNSW → IVF 교체 근거는 ARM CPU의 메모리 접근 패턴 차이다: HNSW 그래프 탐색은 캐시 비친화적이고 IVF는 클러스터 사전 분할로 탐색 공간을 좁혀 검색 시간을 0.07s로 단축했다. LoRA의 파라미터 격리(원본 동결 + delta 행렬)는 Catastrophic Forgetting을 부분적으로 방지하지만 도메인 데이터 비율 90% 이상에서는 불충분하다 — 일반 한국어 20% 리플레이가 필수 조건이었다. T1 LRU(200쿼리, <1ms) → T2 SQLite(30일, ~5ms) → T3 FAISS IVF(~70ms)의 3단 캐시 계층으로 전체 55% 요청이 FAISS를 우회해 P99 310ms → 180ms를 달성했다. 경량화는 LoRA·NF4·토큰 Pruning 조합이 필요했다 — 각각 파라미터 수·비트폭·어휘 크기라는 독립적인 병목을 공략하기 때문이다.",
  ...
  },
  {
    ...
    insight: "GAN 학습 불균형(Discriminator 선행 수렴 → Generator Gradient 고갈 → 모드 콜랩스)은 G 학습률 2배 + G:D 업데이트 비율 2:1 설계로 방지했다. Spectral Normalization이 Discriminator의 립시츠 조건을 강제해 Gradient 폭발을 막는 역할을 한다. Pixel-level L1 Loss만 사용하면 저주파 구조는 재현하지만 뼈 경계·장기 윤곽 같은 고주파 세부 구조가 블러된다 — VGG Perceptual Loss가 feature map 수준 의미 구조를 보존해 해부학적 경계 유실률 30% 감소에 직결됐다. Unpaired 데이터로 교차 모달리티 변환이 가능한 근거는 Cycle-Consistency Loss다 — 변환 이미지가 원본 도메인으로 재변환될 때 원본을 복원하도록 강제해 페어 없이 두 도메인 간 매핑을 학습한다.",
  ...
  },
  {
    ...
    insight: "실시간 안전 서비스의 아키텍처 제약은 피크 시점의 네트워크 불신뢰성이다 — 서비스가 가장 필요한 순간에 연결이 끊길 수 있으므로 오프라인 SQLite 캐시는 선택이 아닌 필수 구조다. FP-Growth 연관 규칙 추출은 단순 거리 기반 추천이 포착하지 못하는 '이 방문 패턴에서 실제로 이동하는 경로'라는 잠재 선호를 데이터에서 직접 뽑는다 — 빈출 아이템셋이 추천 설득력의 근거가 된다. 단일 지표 추천의 극단성(혼잡도만 → 빈 공간, 거리만 → 목적이 다른 장소)은 혼잡도(40%)·거리(30%)·장소 유사도(20%)·시간대(10%) 4개 정규화 가중합으로 균형을 잡는다 — 가중치는 서비스의 핵심 목적인 밀집 분산을 반영해 설계했다.",
  ...
  },
  {
    ...
    insight: "GPT-4.1 단일 Temperature로는 분석 정확도와 전략 창의성을 동시에 확보할 수 없다 — T=0.5 중간값이 두 목표 모두에서 T=0.1/T=0.7 단독보다 낮았다. 역할 자체를 분리해 각자 최적 파라미터에서 동작하는 2-Stage 구조가 유일한 해결책이었다. 멀티 에이전트 파이프라인에서 앞 단계 출력을 암묵적으로 신뢰하면 환각이 파이프라인 전체로 증폭된다 — GPT-4.1 생성 수치가 Gemma-2로 전달될 때 ±20% fact-check 레이어 없이는 최종 리포트의 23%에서 실제 데이터와 ±35% 이상 차이가 났다. 분석가 출력을 그대로 전략가에게 전달하면 포맷 구속으로 전략 옵션이 평균 2.4개로 수렴한다 — JSON 중간 표현으로 정규화해 사실 데이터만 전달하면 5.1개로 증가했다. asyncio 배치 병렬 처리의 안전 배치 크기는 API TPM 상한 역산으로 결정한다 — 배치 40 이상에서 레이트 리밋 오류가 발생해 배치 20이 최대 안전 단위였고, 전체 시뮬레이션 시간이 4분 22초 → 48초로 단축됐다.",
  ...
  },
  {
    ...
    insight: "과학 데이터만으로 CPT를 수행하면 Catastrophic Forgetting으로 기초 언어 능력이 퇴화한다 — 일반 도메인 15~20% 혼합이 Forgetting 억제와 도메인 특화의 실험적 균형점이다. VLM 기반 파서(Nougat/Marker)와 일반 PDF 파서의 결정적 차이는 수식 처리 방식이다 — VLM은 이미지로 렌더링된 수식을 LaTeX로 역복원해 손실 없이 파싱하고, 일반 파서는 첨자·분수가 깨진 ASCII로 변환해 학습 데이터 품질 상한 자체가 다르다. 단일 보상 스칼라 RLHF에서 사실 정확도·형식 준수·응답 간결성은 하나가 올라가면 다른 것이 내려가는 상충 관계다 — GDPO의 그룹 내 독립 정규화가 각 보상 신호를 별도로 최적화해 보상 붕괴 없이 다차원 정렬을 달성하는 메커니즘이다.",
  ...
  },
  {
    ...
    insight: "RAG 품질의 결정 변수는 임베딩 모델이 아니라 청크 전략이다 — 크기 512→256 토큰 축소와 overlap 32 설정이 문항 품질을 더 크게 바꿨다. 강의별 독립 FAISS 인덱스 분리는 수업 단위 네임스페이스 관리가 목적이다 — 단일 통합 인덱스에서는 의미 유사 청크가 다른 강의 검색에 혼입되는 오염이 발생했다. temperature 1.0→0.3 조정만으로는 재현성 확보에 부족하다 — 동일 (강의 ID, 청크 해시) 조합의 24시간 캐시 레이어 추가가 재요청 시 응답 시간 3.2s→0.08s와 문항 완전 일치율 100%를 동시에 달성했다. LLM의 '요약하라'는 지시는 출력 구조를 보장하지 않는다 — '핵심 개념·원리·예시'의 3단 계층을 JSON 스키마로 명시해 각 필드를 강제 채우게 하는 구조화 프롬프팅이 원문 단순 반복을 설명적 텍스트 생성으로 전환시켰다.",
  ...
  },
  {
    ...
    insight: "Next.js 15 App Router에서 Zustand 스토어 초기화는 서버 렌더링 경로에서 격리되어야 한다 — window 객체 접근이 불가한 서버 렌더링 시점에 스토어가 초기화되면 클라이언트 mount 이후 상태 불일치로 Hydration Mismatch가 발생한다. useEffect 내 클라이언트 전용 초기화로 서버 렌더링 경로가 스토어를 건드리지 않도록 격리하는 것이 해결책이었다. 명령형 SDK(Kakao Maps)를 선언형 React에 통합하는 패턴은 useRef + useEffect 캡슐화다 — 마커 인스턴스를 Ref로 관리하고 명령형 생성·제거 로직을 Effect 안에 격리하면 지도 인스턴스 재생성 없이 마커만 갱신해 화면 깜빡임을 막는다. 모바일 터치 이벤트에서 탭과 스크롤 충돌은 좌표 델타 임계값(10px) 필터로 분리한다 — touchstart·touchend 좌표 이동량이 임계값 이하일 때만 tap으로 처리해 onClick의 300ms 지연 없이 의도 판별이 가능하다.",
  ...
  },
  {
    ...
    goal: "회사 문서 인식 엔진의 낮은 정확도를 '감'이 아니라 측정으로 개선 — 어느 단계가 문제인지 분리 측정할 수 있는 채점 체계를 만들고, 그 근거로 인식 구조 전환과 모델 학습·운영 설정을 결정",
    myRole: "품질 진단 체계 설계 · 정답지 딸린 합성 테스트 문서 자동 생성 · 단계별 병목 측정 실험 설계·실행 · 모델 파인튜닝 실험 · 운영 설정 확정",
    desc: "회사 제품인 문서 인식 엔진은 증명서·진단서 사진을 받아 이름·날짜·금액 같은 항목을 자동으로 뽑아 시스템에 넣어줍니다. 제가 받은 과제는 '가끔 틀린다'는 현상뿐이었습니다. 첫 단계로 실제 문서 61건을 5회씩 반복 실행하는 자동 채점 도구를 만들어 오류를 '항상 같은 오답'과 '실행할 때마다 달라지는 오답'으로 나눴고, 실패 케이스의 원본 이미지를 직접 열어보며 문제가 AI의 '행동'이 아니라 AI에게 주는 '입력'에 있다는 걸 확인했습니다. 다음 병목은 채점 그 자체였습니다. 이 엔진은 ①값이 어디 있는지 찾고(위치) ②글자를 읽고(인식) ③읽은 값을 올바른 항목에 연결하는(연결) 세 단계로 일하는데, '최종 정확도 34%'라는 숫자는 셋 중 무엇이 문제인지 알려주지 않습니다. 실제 문서에는 정답지가 없어 단계별 채점이 불가능했기 때문에, 정부가 공개한 법정 서식에 가짜 값을 채워 넣으면서 '몇 번째 칸에 뭘 적었는지'를 동시에 저장하는 방식으로 정답지 딸린 합성 문서 420장(4,732문항)을 만들었습니다. 이제 각 단계에 정답을 대신 넣어보면 병목이 분리됩니다: 위치 정답을 주면 글자 인식은 85점, 위치·글자를 다 줘도 항목 연결은 79점이 상한, 전부 스스로 하면 34점 — 결론은 '글자를 못 읽는 엔진'이 아니라 '위치를 못 찾고 칸을 잘못 잇는 엔진'이었습니다. 이 진단을 근거로 인식 방식을 문서 전체를 보고 이해하는 AI(VLM)로 바꿔 같은 시험지에서 92점을 만들었고, 이후 모델 파인튜닝은 소규모 검증을 통과한 것만 본 학습으로 올리는 승격 심사 구조로, 운영 설정 변경은 스윕 실험의 수치 근거로만 확정하는 체계로 운영했습니다.",
    bullets: [
      "실제 문서 61건을 5회씩 자동 반복 채점하는 도구를 만들어 '항상 틀리는 문제'와 '가끔 틀리는 문제'를 분리 — 손글씨 칸은 그 부분만 잘라 확대해 주는 전처리로 10번 중 0번→10번 정답, 응답 6.3초→1초 안팎",
      "정부 공개 서식으로 정답지 딸린 합성 문서 420장(4,732문항)을 자동 생성하고 단계별 정답 주입 실험 실행: 위치 정답을 주면 글자 인식 85%, 전부 스스로 하면 34% → 병목은 인식이 아니라 위치·연결임을 증명, 문서 전체를 이해하는 AI(VLM)로 전환해 92%",
      "새 서식에서 점수가 반토막(0.86→0.34) 났을 때 모델보다 정답지를 먼저 감사 — 오답 처리 727건이 실은 정답지의 표기 방식 차이였음을 밝혀 재학습 없이 다시 채점만으로 0.76 복구",
    ],
    highlights: [
    ...
    decisions: [
      {
        ...
        reason:
          "기존 엔진은 OCR로 글자를 읽은 뒤 '라벨 오른쪽 칸에 있는 게 값'이라는 위치 규칙으로 항목을 연결합니다. 합성 시험지 4,732문항에서 이 방식은 34.1%였고, 위치와 글자를 정답으로 대신 넣어줘도 연결 단계가 78.5%를 넘지 못했습니다 — 규칙 기반 연결의 구조적 상한입니다. 반면 이미지 전체를 보고 '어느 라벨에 어느 값이 붙는지 판단하라'고 지시한 VLM은 같은 시험지에서 91.7%였습니다. 흥미로운 반전은, VLM에게 OCR이 읽은 글자를 '힌트'로 같이 주면 오히려 점수가 떨어졌다는 것(90.5%→83.0%) — 도와주려고 넣은 정보가 방해가 됐습니다. 기존 OCR 파이프라인은 버리지 않고 병목을 측정하는 기준선으로 남겼습니다.",
      },
      {
        tech: "단계별 정답 주입 사다리 vs 최종 점수 하나로 평가",
        reason:
          "'34%'라는 최종 점수는 위치·인식·연결 중 무엇을 고쳐야 하는지 알려주지 않습니다. 그래서 실험을 사다리처럼 설계했습니다: 전부 스스로 하게 하기 → 위치만 정답 주기 → 위치와 글자 다 주기 — 한 단계씩 정답을 대신 넣으면 점수 차이가 곧 그 단계의 책임이 됩니다. 위치를 정답으로 주면 글자 인식이 85.1%인데 스스로 찾은 위치에서는 73.1%로 떨어졌습니다 — 위치를 잘못 잡으면 인식까지 연쇄로 무너진다는 뜻입니다. 모든 것을 정답으로 주는 조건은 정의상 100%라 결과표에서 빼고, 채점기가 제대로 작동하는지 확인하는 용도로만 썼습니다. 부수 성과로 연결 실패의 절반이 알고리즘이 아니라 정답지 쪽 좌표 누락 때문임도 드러났습니다.",
      },
      {
        tech: "가짜 값을 채운 합성 문서 vs 실제 문서에 사람이 정답 표시",
        reason:
          "실제 문서에 사람이 일일이 정답을 표시하면 건당 5~10분, 필요한 150~200건이면 12~33시간이 들고 검수 도구도 없었습니다. 대신 정부가 공개하는 법정 서식을 내려받아 프로그램으로 가짜 값을 채우면서, 채우는 순간의 좌표를 그대로 정답으로 저장했습니다 — 사람 손을 거치지 않으니 위치 정답이 태생적으로 정확합니다. 서식 PDF가 입력 가능한 양식이 아니어서 표의 선 좌표를 읽어 격자를 복원하고 라벨 위치를 기준으로 값이 들어갈 자리를 계산했습니다. 스캔 문서처럼 보이게 하는 품질 저하(열화)는 '인쇄·노화'와 '촬영' 두 축으로 나눠 문서당 6장을 만들었는데, 섞어버리면 틀렸을 때 원인을 알 수 없기 때문입니다. 그리고 자동 생성 데이터로 만든 모델을 자동 생성 데이터로만 평가하는 순환을 막기 위해, 최종 성능은 반드시 사람이 확인한 실제 문서에서만 인정한다는 원칙을 문서로 못 박았습니다.",
      },
      {
        tech: "이미지를 잘라 확대해 주는 전처리 vs 프롬프트·후처리 보강",
        reason:
          "틀리는 항목마다 프롬프트에 지시를 추가하거나 결과를 규칙으로 고치는 방식부터 검토했지만, 특정 문서에서만 통하는 규칙이 쌓이는 데다 실측에서 효과가 0이었습니다(10번 중 10번 실패 그대로). 전환점은 실패 케이스의 원본 이미지를 직접 열어본 것이었습니다 — 공통적으로 '그 정보가 페이지에서 너무 작거나 읽기 어려운 상태'였습니다. AI가 이상한 게 아니라 AI에게 준 입력이 나빴던 겁니다. 해당 부분만 잘라 확대해 주는 전처리를 넣자 손글씨 항목이 10번 중 0번에서 10번 전부 정답으로 뒤집혔고, 응답 시간도 6.3초에서 1초 안팎으로 줄었습니다. 이어진 발견: 손글씨는 좁게 자를수록, 인쇄 글자는 주변 맥락을 포함해 넓게 자를수록 잘 읽혔습니다. '주변 맥락이 진짜 원인인가'를 확인하려고 글자는 그대로 두고 옆에 도장만 합성해 넣는 실험을 했는데 0/10이 10/10으로 바뀌었습니다 — 맥락이 원인이었습니다.",
      },
      {
        tech: "출력 길이 상한 4096 승인 vs 3072에서 멈추기",
        reason:
          "엔진이 한 번에 출력할 수 있는 길이(토큰) 상한이 1,100으로 설정돼 있었는데, 실제 문서의 답은 2,000~3,000토큰이라 10장 중 7~10장이 중간에 잘렸습니다. 상한을 단계별로 올려보는 실험에서 파인튜닝 모델은 3,072면 충분했지만(4,096과 점수 동일, 속도는 22% 빠름) 기본 모델은 4,096까지 점수가 계속 올랐습니다. 어느 모델을 쓸지 미확정인 상태라 안전한 4,096으로 승인하되 '파인튜닝 모델 확정 시 3,072로 내릴 수 있음'을 조건으로 남겼습니다. 정상 문서는 답이 끝나는 지점에서 알아서 멈추므로 상한을 올려도 추가 비용이 없다는 점도 근거였습니다. 한편 같은 말을 무한 반복하는 고장은 상한을 4배 올려도 한 건도 안 고쳐졌습니다 — 길이 문제가 아니라 구조적 고장이라 별도 과제로 분리했습니다.",
      },
    ],
    experiments: [
      {
      ...
      },
      {
        ...
        conclusion: "정답지 표기 규칙을 정비하고 다시 채점만 해서 0.34→0.76 — 모델은 억울했다"
      },
      {
        id: "D5",
        hypothesis: "학습 데이터를 더 넣으면 약한 유형(긴 문단 항목)이 좋아진다",
        result: "문단 정확도는 0.56→0.97로 급등. 그러나 같은 서식의 다른 항목들이 무너지고 기존 시험 점수도 허용선 아래로 하락 — 승격 심사 7개 기준 중 4개 탈락.",
        conclusion: "배포 보류. 원인 추적 결과 붕괴의 99%는 표기 규칙 확정 전에 만든 학습 데이터가 옛 표기를 담고 있던 것 — 데이터 정정 후 재학습은 별도 승인으로"
      },
      {
        id: "D6",
        hypothesis: "출력 길이 상한을 올리면 같은 말을 반복하는 고장도 사라진다",
        result: "상한을 4배 올려도 반복 고장 3건 중 해결된 건 0건.",
        conclusion: "길이 예산의 문제가 아닌 구조적 고장 — 반복을 감지해 조기에 멈추는 별도 과제로 분리"
      },
    ],
    troubleshooting: [
      { title: "Ghost Improvement — 꺼놓은 코드가 계속 '효과'를 내다", body: "보정 코드를 추가하자 실패하던 항목이 10번 전부 정답이 됐다. 그런데 확인차 그 코드를 꺼도 결과가 그대로였다. 서버에 구분 가능한 표시값을 심어 호출해 보니 응답에 나타나지 않았다 — 서버가 코드 변경을 반영하지 못하는 상태로 계속 떠 있었고, 프롬프트 설정 파일은 서버 시작 때 한 번만 읽는 구조라 파일을 고쳐도 반영되지 않았다. 재시작 후 다시 검증하자 '보정 코드 덕분'이라던 개선이 실은 프롬프트 수정만으로 이미 달성된 것이었음이 드러났다. 이후 '코드가 맞는가'와 '서버에 반영됐는가'를 따로 검증하는 절차를 만들었다." },
      { title: "Session Drift — 같은 조건인데 결과가 정반대", body: "한 항목이 어제는 10번 전부 실패, 오늘은 코드·프롬프트 변경 없이 10번 전부 성공. 원인은 API에 남아 있던 미세한 무작위성(temperature 0.1)이 호출 단위가 아니라 접속 세션 단위로 쏠릴 수 있다는 것이었다. 하루의 반복 실험만 믿고 '항상 틀리는 문제'로 확정했다가 하루 만에 뒤집힌 셈이다. 이후 세션을 바꿔 재확인하고 30회 확정 실행을 최종 판정 근거로 삼는 절차를 추가했으며, 처음의 오판도 방법론의 사각지대 기록으로 그대로 남겼다." },
      { title: "Silent OOM — 에러 메시지 없이 죽는 학습, 원인이 세 겹", body: "모델 학습이 GPU 메모리 27.16GB(물리 한계 24GB 초과)를 찍고 에러 메시지도 없이 죽었다. 파헤치니 원인이 세 겹이었다. ① 손실 계산이 전체 출력을 한 번에 고정밀로 펼쳐 메모리를 폭식하고 있었다 ② 그런데 고친 뒤에도 로그에는 여전히 27.16GB — 측정 함수가 '프로세스 시작 이후 역대 최고치'를 반환하는데 매번 초기화를 안 해 옛 기록이 계속 찍힌 것 ③ 마지막으로 테스트용 샘플이 하필 전체에서 가장 긴 문서였다. 세 가지를 모두 고치자 27.16GB→12.2GB로 안정화됐고 같은 문제는 재발하지 않았다." },
      { title: "Metric Reversal — 성공률을 실패율로 읽을 뻔한 사고", body: "다음 실험의 우선순위 근거로 '문단 항목 실패율 55.6%'를 보고서 초안에 적었다. 그런데 집계 함수의 코드를 열어 반환값 정의를 확인하니 성공÷전체 — 55.6%는 실패율이 아니라 성공률이었다. '숫자가 낮으니 실패율이겠지'라는 추정이 잘못된 실험 설계로 이어지기 직전이었다. 이후 수치를 인용할 때는 그 값을 만든 함수의 정의를 코드에서 재확인하는 것을 절차로 만들었고, 재분석에서 '문서 18종 중 13종은 이미지 품질에 따라 성공과 실패를 오간다'는 더 쓸모 있는 사실을 얻었다." },
      { title: "False Collision — 정답지 일괄 수정 도구의 가짜 충돌 96건", body: "'한 문서 안에 같은 항목 이름이 두 번 나오면 안 된다'는 검사가 첫 실행에서 충돌 96건을 보고했다. 원인은 두 항목이 이름을 서로 맞바꾸는 수정(환자 주소 ↔ 병원 주소)에서 한쪽의 '수정 후' 이름을 다른 쪽의 '수정 전' 이름과 비교한 것 — 수정 도중의 중간 상태를 최종 상태처럼 취급한 버그였다. 모든 항목의 최종 이름을 먼저 계산해 확정한 뒤 확정값끼리만 비교하도록 고치자 96건이 전부 사라졌다. 진짜 충돌은 0건이었다." },
    ],
    insight: "최종 점수 하나는 병목이 어디인지 숨긴다 — 단계마다 정답을 대신 넣어보는 실험 설계가 원인을 분리한다. 모델을 의심하기 전에 채점 기준부터 감사하라 — 점수 반토막의 72%는 정답지 표기 문제였고, 다시 채점만 해서 0.34→0.76이 됐다. 목표 지표가 크게 올라도 다른 지표가 무너지면 배포하지 않는다 — '데이터를 더 넣으면 좋아진다'는 직관을 승격 심사 기준 7개와 통제실험이 대체했다. 자동 생성 데이터로 만든 모델을 자동 생성 데이터로 평가하지 않는다 — 최종 수치는 사람이 확인한 실제 문서에서만 인정한다.",
