...
function ProjectCard({ p, idx }: { p: Project; idx: number }) {
  ...
  // 카드 표시용 필드 — 없으면 상세 콘텐츠로 대체
  const cardTitle = p.card?.topic ?? p.title;
  ...
  return (
    ...
    >
      ...
      <a href={`/projects/${p.slug}`} className="pcard-thumb-link" aria-label={`${p.title} 케이스 스터디`}>
        <div className="pcard-thumb" style={{ ...(thumbBg ? { background: thumbBg } : {}), ...(!p.videoUrl && !thumbMap[p.num] && !thumbImg ? { height: "auto" } : {}) }}>
          {p.videoUrl ? (
            <video
              src={p.videoUrl}
              autoPlay={idx < 2}
              ...
              style={p.card?.thumbAutoHeight ? { width: "100%", height: "100%", objectFit: "contain", display: "block" } : undefined}
            />
          ) : thumbImg ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={thumbImg} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          ) : (
      ...
      <div style={{ padding: "24px 24px 0", display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <h3 style={{ fontSize: 19, fontWeight: 600, lineHeight: 1.35, letterSpacing: "-0.02em", marginBottom: 9 }}>
          <a href={`/projects/${p.slug}`} className="pcard-title-link">{cardTitle}</a>
        </h3>
        ...
        <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 5, marginBottom: 18 }}>
          ...
          {extraTags > 0 && (
            <span style={{ fontFamily: "var(--font-sans)", fontSize: 12.5, color: "var(--ink-light)", alignSelf: "center" }}>
              +{extraTags}
            </span>
          )}
        </div>
  ...
  return (
    <section id="projects" style={{ background: "var(--bg)" }}>
      <div className="sec-wrap">
        ...
        {/* Research section */}
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 48, marginBottom: 32 }}>
          <h3 style={{
            fontFamily: "var(--font-sans)", fontSize: 19, fontWeight: 600,
            color: "var(--ink)", letterSpacing: "-0.02em", lineHeight: 1.35, margin: 0,
          }}>
            연구 프로젝트
          </h3>
        </div>
      ...
      <style>{`
        ...
        .pcard-title-link:hover { color: var(--accent-text); }
        .pcard-link {
        ...
        .pcard-link--primary { color: var(--accent-text); font-weight: 600; }
        .pcard-link:hover { color: var(--accent-text); }
