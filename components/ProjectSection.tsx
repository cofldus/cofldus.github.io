...
const projects: Project[] = [
  ...
  },
  {
    ...
    desc: "단일 벡터 검색만 쓰니 전문 의학 용어에서 정답 문서를 놓쳤고, 검색된 문서를 전부 컨텍스트에 넣으면 LLM이 노이즈에 흔들렸습니다. 두 문제를 2단계 구조로 나눠 처리했습니다. BM25가 용어 매칭을, FAISS가 의미 유사성을 담당해 후보를 넓게 확보한 뒤, bge-reranker가 질문과의 정합성 기준으로 Top-5까지 줄여 LLM에 넘깁니다. 5.5만 건 전문 의학 JSON(약 2.2억 토큰) 코퍼스를 다뤘습니다.",
    bullets: [
      "하이브리드 Retrieval 파이프라인: BM25(Lexical, k=20) + FAISS 벡터 인덱싱(Semantic, k=20) 병렬 검색으로 정답 문서가 후보에 포함되는 비율 개선",
      "BAAI/bge-reranker-v2-m3 리랭킹: 40개 후보를 Top-5로 압축해 LLM이 받는 컨텍스트의 노이즈 감소",
  ...
  },
  {
    ...
    desc: "야민정음('머 → 뫄')과 음운 오류('됩니다 → 됩니당')는 복원 방식이 다릅니다. 야민정음은 모양이 비슷한 자모를 되돌려야 하고, 음운 오류는 발음 변형을 원래 표기로 되돌려야 합니다. 단일 모델로 둘을 같이 학습시키자 혼재 문장에서 출력이 불안정했고 데이터를 3배 늘려도 나아지지 않았습니다. 유형을 먼저 분류하고 전용 복원 모델로 나눠 보내는 구조로 바꾸고 나서야 안정됐습니다.",
    bullets: [
  ...
  },
  {
    ...
    bullets: [
      ...
      "환각 억제 설계: 군집 평균 패턴·리스크 태깅을 GPT-4 컨텍스트에 주입, LLM은 해설 전담 → 환각 80% 감소 (수치 직접 연산 포함 비율, Before/After 50건 수동 측정)",
  ...
  },
  {
    ...
    bullets: [
      "2-Stage GPT: Temp 0.1 '분석가' 모듈이 수치를 정리한 뒤 Temp 0.7 '전략가' 모듈이 제안을 생성하도록 순차 처리",
  ...
  },
  {
    ...
    topic: "증명서·진단서 사진에서 이름·날짜 같은 항목을 자동으로 뽑아내는 문서 AI 엔진의 정확도 개선",
    oneliner: "정부 공개 서식으로 정답지 딸린 합성 문서를 만들어 엔진이 어느 단계에서 틀리는지 분리 측정하고, 인식 방식을 바꿔 같은 시험지에서 34%→92%를 만든 실무 프로젝트",
    date: "2026.07–",
    period: "문서 AI 기업 실무",
    desc: "회사의 문서 인식 엔진은 증명서 사진에서 이름·날짜·주소 같은 항목을 뽑아냅니다. '정확도 34%'라는 숫자만으로는 글자를 잘못 읽은 건지, 글자는 맞는데 엉뚱한 칸에 넣은 건지 구분되지 않았습니다. 실제 문서에는 정답지가 없어 단계별 채점이 불가능했기 때문에, 정부 공개 법정 서식에 가짜 값을 채우면서 그 좌표를 동시에 저장하는 방식으로 정답지 딸린 합성 문서 420장, 4,732문항을 만들었습니다. 단계마다 정답을 대신 넣어보니 병목은 글자 인식이 아니라 위치 탐지와 항목 연결이었고, 이 진단을 근거로 인식 방식을 문서 전체를 보고 이해하는 VLM 기반으로 바꿔 같은 시험지에서 92%를 만들었습니다.",
    bullets: [
      "실제 문서 61건을 5회씩 반복 채점하는 도구로 항상 틀리는 오류와 실행마다 달라지는 오류를 분리, 손글씨 항목은 해당 영역만 잘라 확대하는 전처리로 10번 중 0번→10번 정답",
      "단계별 정답 주입 실험: 위치 정답을 주면 글자 인식 85.1%, 전부 스스로 하면 34.1% → 병목이 위치 탐지와 항목 연결임을 확인하고 VLM 관계 명시형 KIE로 전환해 91.7%",
      "새 서식에서 점수가 0.86→0.34로 떨어졌을 때 정답지를 먼저 감사, 오답 727건이 표기 방식 차이였음을 확인하고 재추론 없이 다시 채점만으로 0.76 복구",
    ],
