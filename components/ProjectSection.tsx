...
const projects: Project[] = [
  {
    ...
    date: "2026.07–",
    period: "문서 AI 기업 실무",
    desc: "회사 제품인 문서 인식 엔진은 증명서와 진단서 사진에서 이름, 날짜, 금액 같은 항목을 추출합니다. 이 엔진은 값의 위치를 찾고, 글자를 읽고, 읽은 값을 항목에 연결하는 세 단계로 동작하는데, 최종 정확도 34%라는 값만으로는 어느 단계에서 손실이 발생하는지 알 수 없었습니다. 실제 문서에는 정답 좌표가 없어 단계별 측정이 불가능했기 때문에, 정부 공개 법정 서식에 가짜 값을 채우면서 그 좌표를 함께 저장하는 방식으로 정답지가 포함된 합성 문서 420장, 4,732문항을 만들었습니다. 단계마다 정답을 주입해 측정한 결과 글자 인식보다 위치 탐지와 항목 연결에서 손실이 컸고, 이를 근거로 인식 방식을 VLM 기반으로 변경했습니다.",
    bullets: [
      "실제 문서 61건을 5회씩 반복 실행하는 채점 도구로 항상 같은 오답이 나오는 항목과 실행마다 달라지는 항목을 구분, 손글씨 항목은 해당 영역만 잘라 확대하는 전처리로 10회 중 0회에서 10회 정답",
      "정부 공개 서식으로 정답지가 포함된 합성 문서 420장(4,732문항)을 생성, 단계별 정답 주입으로 손실 구간 분리 측정(위치 정답 시 인식 85.1%, 전부 예측 시 34.1%) 후 VLM 관계 명시형 KIE로 전환해 91.7%",
      "새 서식에서 F1이 0.86에서 0.34로 떨어졌을 때 정답지를 먼저 검토, 오답 727건이 표기 방식 차이였음을 확인하고 재학습 없이 재채점만으로 0.76 회복",
    ],
    tags: ["Qwen-VL", "vLLM", "PaddleOCR", "LoRA", "합성 데이터셋"],
  ...
  },
  {
    ...
    tags: ["BM25", "FAISS", "bge-reranker-v2-m3", "EXAONE-3.5", "LangChain"],
  ...
  },
  {
    ...
    tags: ["KoELECTRA", "KoBART", "JAMO 전처리", "PyTorch"],
  ...
  },
  {
    ...
    tags: ["Qwen2.5", "LoRA", "4-bit 양자화", "FAISS", "SQLite"],
  ...
  },
  {
    ...
    tags: ["CycleGAN", "VGG Perceptual Loss", "Spectral Norm", "PyTorch", "DICOM"],
  ...
  },
  {
    ...
    tags: ["FP-Growth", "SQLite", "Naver Maps API", "Python"],
  ...
  },
  {
    ...
    tags: ["GPT-4.1", "LLM Agent", "Gemma-2-9b", "asyncio", "Flask"],
  ...
  },
  {
    ...
    tags: ["CPT", "SFT", "GDPO", "Nougat/Marker", "Nemotron"],
  },
  {
    slug: "rocketan",
    num: "10",
    ...
    tags: ["LangChain", "FAISS", "RAG", "pdfplumber", "OpenAI API"],
  ...
  },
  {
    ...
    tags: ["Next.js 15", "TypeScript", "Zustand", "WebSocket/STOMP", "Kakao Maps API"],
